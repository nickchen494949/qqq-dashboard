...
hyg_ief = hyg_d / ief_d
ma252   = hyg_ief.rolling(Z_WINDOW).mean()
sd252   = hyg_ief.rolling(Z_WINDOW).std()
...
for fn in sep_pdfs:
    ...
    # Parse ALL numbers on each row (columns = year, year+1, year+2, year+3, ...)
    cp_all, fr_all = [], []
    lines = text.split('\n')
...
# Hardcoded early rates — keyed by (date, target_year)
# Pre-liftoff (2012-2014): ZIRP for all years
_ffr_by_target = {
    ...
    ('2018-12-19', 2019): 2.9,  # PDF text extraction splits table; actual median = 2.9
}
for row in sep_raw:
    ty = row['target_year']
    key = (row['date'], ty)
...
# Generate signals — SAME TARGET YEAR comparison
# Rule: compare current SEP's target_year values against
#        previous SEP's values FOR THE SAME target_year
sep_signals = []
sep_in = True
for i in range(1, len(sep_raw)):
    c, p = sep_raw[i], sep_raw[i-1]
    ty = c['target_year']
    # Current meeting's projections for target year
    c_pce = c['pce_by_year'].get(ty)
    c_rate = c['rate_by_year'].get(ty)
    # Previous meeting's projections for THE SAME target year
    ...
    same_ty = (ty == p['target_year'])  # flag: same or cross comparison
    signal = ''
...
# FIX: Map signal dates to NEXT trading day (not same day)
sep_signal_dates = {}
unmapped = []
for r in sep_signals:
    ...
    if len(candidates) == 0:
        ...
        continue
    mapped = candidates[0]
    # Signal should execute on NEXT trading day after mapped
    exec_candidates = idx[idx > mapped]
    if len(exec_candidates) == 0:
        unmapped.append(r['date'])
        continue
    exec_date = exec_candidates[0]
...
def run_backtest(daily_returns, gap_returns=None, intra_returns=None,
                 ...
                 vz_trigger=VZ_TRIGGER, vz_recover=VZ_RECOVER, vz_lev=VZ_LEV, tc_bps=25):
    """
    daily_returns: close-to-close returns for normal days
    gap_returns:   prev_close → open (overnight, earned at OLD leverage)
    intra_returns: open → close (intraday, earned at NEW leverage)
    """
    eq = 1.0; lev = 3.0; prev_lev = 3.0
    pending = None; eql = []; levs = []
    in_trade = False; trade_entry_eq = 1.0; in_danger = False; vol_danger = False; trades = 0
    trade_log = []; danger_log = []; vol_danger_log = []
    switch_today = False  # flag: leverage just changed this bar
    ...
    for i in range(len(idx)):
        ...
        switch_today = False
        prev_lev_for_gap = lev  # save old lev for gap calculation
        ...
        if si == 0:
            ...
            if use_overlay:
                ...
                if in_danger:
                    tgt = 1 if is_profitable else 3  # NSL for Credit
                elif vol_danger:
                    if is_profitable: tgt = vz_lev
                    else: tgt = lev  # NSL for Vol: stay where we are (3x or 2x)
                else:
                    tgt = 3
            else: tgt = 3
        ...
        if i > 0:
            r_total = daily_returns.iloc[i]
            if np.isnan(r_total): r_total = 0.0

            # On switch day: split into gap (old lev) + intra (new lev)
            if switch_today and gap_returns is not None and intra_returns is not None:
                rg = gap_returns.iloc[i]
                ri = intra_returns.iloc[i]
                if np.isnan(rg): rg = 0.0
                if np.isnan(ri): ri = 0.0
                # Multiplicative: (1 + old*gap) * (1 + new*intra) - 1
        ...
        prev_lev = lev; eql.append(eq); levs.append(lev); danger_log.append(in_danger); vol_danger_log.append(vol_danger)
    ...
    return {
        'equity': es, 'leverage': levs, 'danger': danger_log, 'vol_danger': vol_danger_log,
...
_audit_src = open(__file__).read()
_line = [l for l in _audit_src.split('\n') if l.strip().startswith('has_both') and 'all' in l]
