...
# 6. Rewrite JS
js_old = """function barColors(yArr) {
...
js_new = """function barColors(yArr) {
  return yArr.map((v, i) => i === 0 ? '#A6B0C3' : (v > yArr[i-1] ? '#FF333A' : '#00C805'));
}
...
# Because Javascript curly braces mess up f-strings, we use standard string replace
if js_old in text:
    text = text.replace(js_old, js_new)
else:
    print("JS_OLD NOT FOUND")
